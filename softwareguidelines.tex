...
%within which the issue of software quality is placed, 
...
\begin{itemize}
	...
	\item additional guidelines %TODO-rdv hier moeten we  misschien een andere term dan guidelines gebuiken. Ik heb zitten denken: protocols? instructions? procedures? van de andere kant: als we dit doen moeten we in het hele document doorvoeren... misschien de moeite niet waard
\end{itemize}
...
The aim of this document %TODO-rdv aim waarvan? dit document? 
is, in an iterative
process, to tailor the guidelines to the needs of the involved partners (ICT, research and archives). 
%For the context of questions of software quality in SSH research infrastructures see the last section in this document.
...
% TODO-rdv: issue #2. open vragen vs meetbare punten: likertschaal + toelichting  
...
\newcommand{\sixteen}{If the software contains a configuration file, then the root of the repository must contain a \textbf{configuration file containing all options} set to the sane defaults or commented out containing a description when you want to use them.} %TODO-rdv deze snap ik niet
...
\newcommand{\eighteen}{The software must have a \textbf{CONTRIBUTING file}. (The template could be used---but it must be noted that that means one must be open to all PRs. Specifying some details in this file thus is very useful.} %TODO-rdv wat zijn PRs?
\newcommand{\nineteen}{Data must be made available as \textbf{a log or rdf quads}.} %TODO-rdv of --> or: OK?
