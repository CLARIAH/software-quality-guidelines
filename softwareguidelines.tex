...
\newcommand{\five}{The software must be able to \textbf{run in the foreground}
after installation.} %MvG: Zoals eerder gemaild denk ik dat deze wat nader toegelicht moet worden
...
\newcommand{\nine}{The root folder of the software project contains a file
    ...
    and HTML are not. A UTF-8 encoding should be chosen; however, the use of
    ANSI, UTf-16 or UCS2 is also acceptable when forced by some technical
    constraint. (Though you must then mention that using the ascii subset at
    the start of the file itself.} %TODO-rdv die laatste zin tussen haakjes snap ik niet   (MvG: De uitzondering is wat mij betreft iets te specifiek, ik zou gewoon utf-8 eisen)
	\newcommand{\nineOne}{The readme must start by explicitly mentioning the experimental nature of the software.}
	\newcommand{\nineTwo}{Accurate support contact information must be provided
...
\refcrit{U4}}
	...
	\newcommand{\nineNine}{The readme must contain either a quick start,
...
above), HTML that does not depend on styles/js, and PDF/A. In the case of
...
\newcommand{\twelveB}{screenshots/screencasts of the application}
\newcommand{\twelveC}{a link to the source code}
\newcommand{\twelveD}{contact information, or a link to the bug tracker}
...
\newcommand{\twelveF}{the website must be registered on Google, and a link must be provided to the project}
\newcommand{\twelve}{The project must have a \textbf{website}, containing, at
    the very least:	 \refcrit{ID2}
%	\begin{itemize}
%		\item \itemA 
%		\item \itemB
%		\item \itemC
...
app, a main page should be provided, or a text file that contains a link to the
home page should be distributed with the binaries. \refcrit{L3}}
...
expected to be called. \refcrit{L4} \refcrit{AN3}}
\newcommand{\sixteen}{If the software depends on configuration files, then a
    template or sample must be provided in the repository  for each, \textbf{containing all
available options} and set to sane defaults. Comments in the configuration file
should give clear descriptions what the options do. \refcrit{L5}} %TODO-rdv deze snap ik niet (MvG: rephrased, nu wel?)
\newcommand{\seventeen}{The software must be \textbf{distributed under an OSI
approved licence} in a LICENSE file in root. \refcrit{CP1} Also copyright
holderi \refcrit{CP2}} %TODO-rdv wat wordt met dat laatste zinnetje bedoeld?
...
\newcommand{\nineteen}{Data must be made available as \textbf{a log or rdf
quads}. \refcrit{IP1}} %TODO-rdv OK? (MvG: Wat mij betreft niet, deze moet een stuk duidelijker worden gemaakt, Jauco?)
\newcommand{\twenty}{The software must \textbf{not store usernames and
passwords} \refcrit{SC2}; instead, it must use the instruments provided by the project.}
\newcommand{\twentyOne}{Experimental software MUST NOT allow external users to
    store data (for this renders it production software).} %MvG: extra toelichting nodig denk ik
...
systems (see \refcrit{AC1}, \refcrit{AC2}) with minimal effort, resulting in an automatic run of the test suite
